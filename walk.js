...
(function () {
 ...
 camera.position.copy(pos); camera.rotation.set(0, 0, 0); camera.rotateY(yaw); camera.rotateX(pitch);
  ...
  const BENCH = [
    ...
    { model: /electric_tank_water_heater/, name: 'the upper thermostat', terms: [[/^upper_stat_t1$/, '1'], [/^upper_stat_t2$/, '2'], [/^upper_stat_t4$/, '4']],
      ohms: { '1-2': ['OL', 'terminal 1 to 2 is the path to the UPPER element. The top of this tank is hot, so that path is open. On a cold tank it reads closed'],
              '1-4': [0.2, 'terminal 1 to 4 is the path down to the LOWER thermostat. With the top of the tank hot the upper stat hands the power down, so this is closed'],
              '2-4': ['OL', '2 and 4 are never joined: the upper thermostat sends the power to one or the other, never both'] } },
    { model: /electric_tank_water_heater/, name: 'the lower thermostat', terms: [[/^lower_stat_t1$/, '1'], [/^lower_stat_t2$/, '2']],
